...
dir_images  = './collapsed_fishing_grounds'
...
def train_model(model, criterion, optimizer, scheduler, num_epochs, dataloaders, dataset_sizes):
    ...
    for epoch in range(num_epochs):
        ...
        logging.info('Diff of acc between two epochs: {:.4f}'.format(epoch_acc - last_acc))
        if (epoch + 1) % 5 ==0 and epoch_acc - last_acc <= 0.02:
            break
    ...
    torch.save(best_model_wts, "./model_fg_rgb_incep_3grp.pt")

    return model

def train(flip_rate, brightness, rd_crop, batch_size, lr, momentum, weight_decay, decay_bd, gamma, num_epochs):
	data_transforms = {
	...
	model_ft = models.inception_v3(pretrained=True).cuda()
...
def main():
    logging.basicConfig(filename='train_fg_rgb_incep_3grp.log', level=logging.INFO)
    logging.info('Started')

    args = config_train_fg.get_args()
    args_dict = vars(args)
    train(**args_dict)
